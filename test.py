...
def load_model_safely(model, checkpoint_path):
    ...
    skipped = [k for k in checkpoint.keys() if k not in filtered_checkpoint or model_state[k].shape != checkpoint[k].shape]
    if skipped:
        print(f"Skipped loading the following layers due to shape mismatch: {skipped}")
...
def main(model_type, val_file, checkpoint_folder, hierarchy, supervised=False, device='cuda:0', batch_size=p.batch_size, limit_rows_val=p.limit_rows_val, subbatches=p.subbatches, num_workers=p.num_workers, email_sender=None, results_folder=None, noload=False):
    ...
    batcher, model, postprocessor = load_model_components(model_type, hierarchy, run_type='testing', device=device, model_file=None, cluster=supervised)
    if model_file and not noload:
        model = load_model_safely(model, model_file)
