...
from transformers import BertTokenizer, AutoTokenizer  # For flex
from functools import partial

class Batcher(StandardBatcher):
    @staticmethod
    def filter_function(text: str, tokenizer: Union[BertTokenizer, AutoTokenizer]) -> bool:
        """
        Filters out strings based on their token length.

        Args:
            text: The string to filter.
            tokenizer: A tokenizer instance (BertTokenizer or AutoTokenizer).

        Returns:
            True if the length of the tokenized string is greater than 4, False otherwise.
        """
        return len(tokenizer.tokenize(text)) > 4
    ...
    def __init__(self, hierarchy, ancestors=False, code_id=False, code_description=False, code_linearization=False, description_linearization=False, description_embedding_linearization=False, resample_neg_proportion=None, counts=None, tfidf_tokenizer=False, add_special_tokens=True):
        ...
        #self.filter = self.create_filter_function(bert_tokenizer)
        #self.filter = lambda x: filter_function(x, bert_tokenizer)
        #self.filter = lambda x: Batcher.filter_function(x, self.bert_tokenizer)
        #self.filter = self.filter_function
        self.filter = partial(self.filter_function, tokenizer=bert_tokenizer)
...
def get_sentences(reports_df, num_sentences=None, filter=lambda x: True, report_max_length=1000000):
    ...
    for i,report in reports_iter:
        ...
        sents = list(nlp(report[5][:report_max_length]).sents)
        if num_sentences is not None and num_sentences < 0:
...
class Instance(StandardInstance):
    def __init__(self, raw_datapoint, tokenizer, codes, hierarchy, ancestors=False, code_id=False, code_description=False, code_linearization=False, description_linearization=False, description_embedding_linearization=False, resample_neg_proportion=None, counts=None, tfidf_tokenizer=False, filter=lambda x: True):
        ...
        if 'targets' in raw_datapoint.keys():
            ...
            #[codes[code_str] for code_str in targets if code_str in codes]
            self.datapoint['codes'] = torch.tensor([codes[code_str] for code_str in targets if code_str in codes])
