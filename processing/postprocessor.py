...
#                   'reference_sentence_indices', 'reference_sentence_rankings', 'reference_sentence_attention']
        ...
        for b in range(len(batch)):
            print('#########')
            print(batch.instances[b]['original_reports'])

            original = batch.instances[b]['original_reports'][0]
            try:
                patient_id = int(original.iloc[0])  # Try to convert the value to int
            except (ValueError, TypeError, AttributeError, IndexError) as e:
                print(f"Skipping row {b}: {e}")
                continue  # Skip this iteration if conversion fails

            patient_id = int(batch.instances[b]['original_reports'][0].iloc[0])
